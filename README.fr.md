...
Intégration de démonstration qui relie **Jira Service Management** à une couche de triage par IA et à un système externe, avec de la gouvernance intégrée.

Quand un ticket arrive dans **Jira Service Management**, un workflow **n8n** (iPaaS)
le récupère, le fait **trier par une IA (Claude)** (catégorie, priorité, sentiment,
résumé, réponse suggérée), réécrit le résultat dans Jira, **escalade les incidents
critiques** vers un système externe (Postgres), et **synchronise les statuts dans
les deux sens** — le tout avec **gouvernance** (logs, idempotence, gestion d'erreurs).
...
> Les concepts iPaaS ici — triggers, connecteurs, mapping de données, gestion d'erreurs — correspondent 1:1 à Workato et Boomi. n8n est la façon open source de les démontrer.
...
```
- n8n : **http://localhost:5678** → au 1er lancement, crée ton compte propriétaire (email + mot de passe).
